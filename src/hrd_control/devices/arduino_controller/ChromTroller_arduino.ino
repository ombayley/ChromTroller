...
// --Constants--
// LCMS Trigger Pins
#define START_REQUEST_SIGNAL_PIN 5 // Pin to output to ERI Remote Pin #1 (White)
#define STOP_SIGNAL_PIN 6 // Pin to output to ERI Remote Pin #2 (Brown)
#define READY_SIGNAL_PIN 7 // Pin to output to ERI Remote Pin #3 (Green)
#define POWER_ON_PIN 8 // Pin to output to ERI Remote Pin #4 (Grey)
#define EMERGENCY_SHUT_DOWN_PIN 9 // Pin to output to ERI Remote Pin #6 (Pink)
#define START_SIGNAL_PIN 10 // Pin to output to ERI Remote Pin #7 (Blue)
#define PREPARE_SIGNAL_PIN 11 // Pin to output to ERI Remote Pin #8 (Red)

// Phase Sensor Read Pins
#define PHASE_SENSOR_OUT_A_PIN 3 // Pin to read phase sensor output (Orange)
#define PHASE_SENSOR_OUT_B_PIN 4 // Pin to read phase sensor output (Blue)
#define PHASE_SENSOR_CALIBRATE_PIN 2 // Pin to calibrate phase sensor (Green)
#define PHASE_SENSOR_ANALOG_PIN 19 // Pin to read phase sensor output (White)

// Valve Control Pins
#define VALVE_A_IN_PIN 17  // Pin to to read valve position A (Vici Pin 3)
#define VALVE_B_IN_PIN 16 // Pin to to read valve position B (Vici Pin 4)
#define VALVE_A_OUT_PIN 15 // Pin to to set valve to position A (Vici Pin 5)
#define VALVE_B_OUT_PIN 14 // Pin to to set valve to position B (Vici Pin 6)
...
void parse_serial(){
    ...
    int int_response = 3;  // Default response is error
        ...
        break;
        case 13:
          // LCMS PREPARE Signal state
          int_response = digitalRead(PREPARE_SIGNAL_PIN);
          Serial.println(int_response);
			}
