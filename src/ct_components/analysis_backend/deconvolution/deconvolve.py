...
from src.ct_components.analysis_backend.deconvolution.nonnegative_lstsq import concentrations_from_spectra, spectra_from_concentrations
from src.ct_components.analysis_backend.deconvolution.fit_peak_model import fit_peak_model
from src.ct_components.analysis_backend.deconvolution.peak_models import PeakModel, BiGaussian, BiGaussianTailing, FraserSuzuki, Bemg, BiGaussianSingle


def deconvolve_adaptive(
        data: NDArray,
        model: PeakModel | Literal['BiGaussian', 'BiGaussianTailing', 'FraserSuzuki', 'Bemg', 'BiGaussianSingle'],
        max_mse: float,
        relaxe_concs: bool,
        min_comps: int,
        max_comps: int
    ...
    if isinstance(model, str):
        model = {
            ...
            'Bemg': Bemg(),
            'BiGaussianSingle': BiGaussianSingle()
        }[model]
