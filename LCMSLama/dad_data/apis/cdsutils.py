...
Takes the single wavelength CSV output from OpenLabs CDS and compiles it into spectra x time data.
This uses the chromatogram extraction feature in OpenLabs CDS v2.7 to generate a chromatogram
at multiple wavelengths which are exported individually and then combined in this script.
This data exporting may change in later updates of OpenLabs CDS.
"""
import os
import re
import glob
...
def make_3d_spectra_chromatogram(path) -> pd.DataFrame:
    """
    Takes a serries of exported CSV chroms and assembles them into a 3D data file and df
    """
...
def interpolate_missing_wavelengths(df: pd.DataFrame, resolution: float) -> pd.DataFrame:
    """
    Interpolates missing wavelength chromatograms in the DataFrame to ensure chromatograms at specified
...
if __name__ == '__main__':
    path = r"C:\Users\obayley\Platform_Data\Olly_Test\Test11\LCMS_data\2024-03-01 18-01-13+01-0020sm_80pr_sample_001.sirslt"
    make_3d_spectra_chromatogram(path)
