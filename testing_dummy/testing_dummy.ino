...
#define PHASE_SENSOR_OUT_A_PIN 7 // Pin to read phase sensor output (Orange)
#define PHASE_SENSOR_OUT_B_PIN 6 // Pin to read phase sensor output (Blue)
...
long time_of_send = 0;
long phase_sensor_data_interval = 200; // send data every 100 ms
...
void send_phase_sensor_data(){
  ...
  if(phase_sensor_a_pin == HIGH && phase_sensor_b_pin == LOW){
    Serial.println("_DSPS-1"); // Clear Solution
  }else if(phase_sensor_a_pin == LOW && phase_sensor_b_pin == HIGH){
    Serial.println("_DSPS-0");  // Empty
  }else if(phase_sensor_a_pin == HIGH && phase_sensor_b_pin == HIGH){
    Serial.println("_DSPS-2"); // Dark Solution
  }else {
    Serial.println("_DSPS-3"); // Error
  }
}
...
void loop()
{
	...
	{
		parse_serial();
    if (millis() - time_of_send > phase_sensor_data_interval){
      send_phase_sensor_data();
      time_of_send = millis();
    }
    /*
