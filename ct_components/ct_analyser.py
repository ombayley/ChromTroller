...
class Analyser:
    ...
    def compare_spectra(self, spectrum1, spectrum2):
        ...
        similarity_percentage = self.get_similarity(norm_abs1, norm_abs2, model="cosine")
    ...
    def save_all_peaks_to_csv(self, peak_data):
        """
        Save all peaks to a CSV file.

        Args:
            peaks (list): A list of peak data dictionaries.
        """
        spectra_list: Optional[List[dict]] = peak_data.get("spectrum")
    ...
    def plot_spectra(self, spectra):
        ...
        plt.plot(spectra.get("wavelength"), spectra.get("absorbance"), label=f'Component')
        plt.title(f'Spectrum of Component')
